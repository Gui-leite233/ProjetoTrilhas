@extends('templates.main', ['menu' => "admin", 'submenu' => "Editar Projeto"])

@section('titulo') Editar Projeto @endsection
        ...
        <div class="col-md-8 mx-auto">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body p-4">
                    <form action="{{ route('admin.projeto.update', $projeto->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea class="form-control @if($errors->has('descricao')) is-invalid @endif"
                                name="descricao" rows="4" required>{{ old('descricao', $projeto->descricao) }}</textarea>
                            ...
                            <select name="user_ids[]" class="form-select" multiple required>
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" 
                                        {{ in_array($user->id, $projeto->users->pluck('id')->toArray()) ? 'selected' : '' }}>
                                        {{ $user->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('admin.projeto.index') }}" class="btn btn-secondary px-4">
